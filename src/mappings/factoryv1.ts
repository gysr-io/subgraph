...
import { createNewStakingToken, createNewRewardToken } from '../util/common';
...
export function handleGeyserV1Created(event: GeyserCreated): void {
  ...
  // pool staking token
  let poolStakingToken = createNewStakingToken(pool, stakingToken);
  pool.stakingTokens = [poolStakingToken.id];

  // pool reward token
  let poolRewardToken = createNewRewardToken(pool, rewardToken);
  pool.rewardTokens = [poolRewardToken.id];
  ...
  log.info('created new pool: geyser v1, {}, {}, {}', [
    pool.id,
    stakingToken.symbol,
    rewardToken.symbol
  ]);
